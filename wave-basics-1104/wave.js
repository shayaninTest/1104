...
function DrawTransWave(t,Yshift) {
    ...
    for (let i = 1; i <= HorParts; i++) {
        ...
        if (i == Math.ceil(HorParts/2)) {
            DrawParticle(particles[i-1].x, particles[i-1].y, 'rgb(255,0,0)'); // draw red particle at the middle
        } else {
            DrawParticle(particles[i-1].x, particles[i-1].y, '#DDF2FF');
        }
    }
}
...
function DrawLongWave(t,Yshift) {
    for (let n = 1; n <= LongRows; n++) {
        ...
        // draw particles at the middle as red when in compression and green when in expansion
        let redness1 = 255*(1 + Math.sin((2*Math.PI/lamda)*((Math.ceil(7*HorParts/13)-1)*spacing)-(2*Math.PI*f)*t+phi-Math.PI/2))/2;
        let greenness1 = 255*(1 + Math.sin((2*Math.PI/lamda)*((Math.ceil(7*HorParts/13)-1)*spacing)-(2*Math.PI*f)*t+phi+Math.PI/2))/2;
        DrawParticle(particles[Math.ceil(7*HorParts/13)-1].x, particles[Math.ceil(7*HorParts/13)-1].y, 'rgb('+ redness1 +','+ greenness1 +',0)');
        
        // draw particles at the middle as red when in compression and green when in expansion
        let redness2 = 255*(1 + Math.sin((2*Math.PI/lamda)*((Math.ceil(6*HorParts/13)-1)*spacing)-(2*Math.PI*f)*t+phi-Math.PI/2))/2;
        let greenness2 = 255*(1 + Math.sin((2*Math.PI/lamda)*((Math.ceil(6*HorParts/13)-1)*spacing)-(2*Math.PI*f)*t+phi+Math.PI/2))/2;
        DrawParticle(particles[Math.ceil(6*HorParts/13)-1].x, particles[Math.ceil(6*HorParts/13)-1].y, 'rgb('+ redness2 +','+ greenness2 +',0)');
    }        
}
